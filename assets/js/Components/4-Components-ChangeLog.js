$('body').on("click", function(e){
    ...
    if (thiselement == `changelog`) {
        ...
        if ($('#container-form > form > div:nth(0) > label').text() == 'DADOS DO CLIENTE') {
            if (clientID == '') {
                PopulateChangeLog('')
            } else {
                PopulateChangeLog(clientID,`client`)
            }
            return
        }
        
        if ($('#container-form > form > div:nth(0) > label').text() == 'DADOS DO PEDIDO') {
            if (requestID == '') {
                PopulateChangeLog('')
            } else {
                PopulateChangeLog(requestID,`request`)
            }
            return
        }
    }    
